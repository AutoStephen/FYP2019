...
// The % difference is taken away from the inner wheel.
// NOTE: "S" is now being sent only in the first 3 seconds ***********************************
  ...
  SoftwareSerial mySerial1(6, 7); // RX, TX, Create a softwareSerial port on pins 10 and 11.
  SoftwareSerial mySerial2(8, 9); // RX, TX, Create a softwareSerial port on pins 12 and 13.
...
void loop() 
{
  while (millis() < 3000) //While in the first 3 seconds, send "s" so the controller definitely knows to be in serial mode.
  {
    mySerial1.println("s"); //Initialise the controller to accept serial command strings.
    mySerial2.println("s"); //Initialise the controller to accept serial command strings.
  }
  //Calculations to store the differential when braking:
...
////////////////////////////////////////////////////////////////////////////////////////////
  ...
  BrakePercent = 0; //Throttle value converted to %.
  ...
  SteeringInput = analogRead(A1); //Value from potentiometer placed into "SteeringInput".
  ...
  analogWrite(MotorLeft, LeftOutput);
  analogWrite(MotorRight, RightOutput);
  

  RightOutput = ((RightMotorPercent / 100.00) * 255.00);
  LeftOutput = ((LeftMotorPercent / 100.00) * 255.00);
  
////////////////////////////////////////////////////////////////////////////////////////////
//ELECTRONIC DIFFERENTIAL ALGORITHM:
 if (OutputPercent < 3 || BrakePercent > 97) //If the throttle signal is 0% or brake is 100%, both motors should be off. 
{
  OutputPercent = 0; //Turn signal to motors off
  RightMotorPercent = 0;
  LeftMotorPercent = 0;
}

//IF DRIVING STRAIGHT
if (SteeringPercent > 48 && SteeringPercent < 52)
{
  ...
  RadiusDifferencePercent = (0); //Outer - inner / outer
  ...
  OverHundredVariable = (OutputPercent + ExcessPercent); //Value past 100%. e.g. 112%
    ...
    if (BrakePercent >= 97.00) 
      ...
      else // else, we are in a fairly straight line: (neither signal is greater than 100 or less than 0)
      {
