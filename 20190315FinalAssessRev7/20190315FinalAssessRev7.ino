...
{ 
  ...
  else if (SteeringPercent < 50) //If turning left (repeat calculations but swap the motors).
  {
    ...
    OverHundredVariable = (OutputPercent + ExcessPercent); //Value past 100%. e.g. 112%
      ...
      else if ((OutputPercent - (RadiusDifferencePercent / 2))) //If signal to motor is less than 0%
      {
        LeftMotorPercent = 0; //Inner motor set to 0%
        RightMotorPercent = ((OutputPercent + (NegativePercent - (NegativePercent*(BrakePercent / 100))) + ((RadiusDifferencePercent - (RadiusDifferencePercent * (BrakePercent / 100))) / 2)));
        //RightMotorPercent = ((OutputPercent + (NegativePercent - (NegativePercent*(BrakePercent / 100))) + (RadiusDifferencePercent / 2)) - ((OutputPercent + NegativePercent + (RadiusDifferencePercent / 2))*(BrakePercent / 100))) ; //Outer motor has the excess % added onto it.
        //OutputPercent = (ThrottlePercent - (ThrottlePercent * (BrakePercent / 100)));
      }
      else 
    {
