...
{ 
  ...
  RadiusDifferencePercent = (((LeftWheelTurningRadius - RightWheelTurningRadius) / LeftWheelTurningRadius) * 100);
  ExcessPercent = (ThrottlePercent + (RadiusDifferencePercent / 2) - 100);
  OverHundredVariable = (ThrottlePercent + ExcessPercent);
  if (OverHundredVariable > 100)
      {
        
        LeftMotorPercent = 100.00;
        RightMotorPercent = (ThrottlePercent - ExcessPercent - (RadiusDifferencePercent / 2));
      }
      else
  {
  LeftMotorPercent = (ThrottlePercent + (RadiusDifferencePercent / 2));
  RightMotorPercent = (ThrottlePercent - (RadiusDifferencePercent / 2));
  }
}
  else if (SteeringPercent < 50) //If turning left
  {
    ...
    RadiusDifferencePercent = (((RightWheelTurningRadius - LeftWheelTurningRadius) / RightWheelTurningRadius) * 100);
    ExcessPercent = (ThrottlePercent + (RadiusDifferencePercent / 2) - 100);
    OverHundredVariable = (ThrottlePercent + ExcessPercent);
      if (OverHundredVariable > 100)
      {
        RightMotorPercent = 100.00;
        LeftMotorPercent = ((ThrottlePercent - ExcessPercent) - (RadiusDifferencePercent / 2));
      }
      else 
    {
    LeftMotorPercent = (ThrottlePercent - (RadiusDifferencePercent / 2));
