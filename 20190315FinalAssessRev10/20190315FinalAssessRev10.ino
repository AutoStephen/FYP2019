  ...
  OverHundredVariable = (OutputPercent + ExcessPercent); //Value past 100%. e.g. 112%
    ...
    if (BrakePercent > 98.00) 
      ...
      else if ((OutputPercent - (RadiusDifferencePercent / 2)) < 0) //If signal to motor is less than 0%
      {
        ...
        LeftMotorPercent = (OutputPercent + (ExcessRadiusDifferencePercent)); //Outer motor has the excess % added onto it.
      }
      else
  {
  ...
  else if (SteeringPercent < 50) //If turning left (repeat calculations but swap the motors).
  {
    ...
    OverHundredVariable = (OutputPercent + ExcessPercent); //Value past 100%. e.g. 112%
      ...
      else if ((OutputPercent - (RadiusDifferencePercent / 2)) < 0) //If signal to motor is less than 0%
      {
        ...
        //ExcessRadiusDifferencePercent = (RadiusDifferencePercent - (RadiusDifferencePercent * (BrakePercent / 100)));
        RightMotorPercent = (OutputPercent + (ExcessRadiusDifferencePercent)); //Outer motor has the excess % added onto it.
      }
      else 
    {
